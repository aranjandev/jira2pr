...
Workflow definitions live in the platform's agent-workflows directory. Read the matching workflow file and follow it step-by-step. Every workflow begins with **Phase 0: Bootstrap**, which handles both FRESH (JIRA input) and RESUME (PR input) modes — all routing, state parsing, and resume logic lives there.
