...
- **Status** — current phase (`Planning` → `Implementing` → `Reviewing` → `Ready`), draft flag, last updated timestamp- **Links** — JIRA ticket URL, branch name (used by agents to resume and navigate)- **Intent** — problem, desired outcome, non-goals, constraints (immutable after plan approval)
