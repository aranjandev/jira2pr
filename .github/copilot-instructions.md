...
a ready-to-use setup for their tool (e.g., VS Code Copilot), then 
...
- Key directories:
  - `canonical/` — Platform-agnostic source of truth for all agent concepts: agents, skills, prompts, workflows, instructions, and model tiers. **Edit here, not in the output folders.**
    - `canonical/platform-extras/` — Platform-specific fragments that the assembler merges in during generation
  ...
  - `examples-autogenerated/opencode/` — Generated output for OpenCode (reference only; safe to delete and regenerate)
...
python scripts/assemble.py --target-dir examples-autogenerated/opencode        --platform opencode
...
- Prompt source files in `canonical/prompts/` are plain Markdown — assembled into `.prompt.md` files for Copilot and platform-specific formats for other tools.
